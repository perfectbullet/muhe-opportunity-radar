...
4. **📊 双前端界面**：
   - **Vue3 前端（推荐）**：现代化 Web UI，支持单一分析、多视角对比、历史记录、统计信息
   - **Gradio 前端**：快速原型，支持基础分析功能
5. **📄 文档管理与分析（新功能）**：
   - 支持 PDF、Word、Markdown 文件上传
   - LangGraph 智能工作流分析（解析→计算→分析→汇总）
   - 自动提取财务指标（PE、PB、ROE、PEG 等）
   - 多投资者视角分析报告
6. **💾 历史记录管理**：MongoDB 存储分析历史，支持回溯和趋势分析
## 快速启动
