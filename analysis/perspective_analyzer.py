...
try:
    ...
    print("⚠️  LangChain 未安装，请运行: pip install langchain langchain-openai langchain-community")
...
class PerspectiveAnalyzer:
    ...
    def __init__(
        ...
        Args:
            llm_provider: LLM提供商 (deepseek/qwen/zhipu/openai/claude)
            api_key: API密钥，如果不提供则从环境变量读取
            model_name: 模型名称，如果不提供则使用默认模型
            temperature: 温度参数，控制输出的随机性
        """
        if not LANGCHAIN_AVAILABLE:
            raise ImportError("需要安装 LangChain 库")
    ...
    def _init_llm(self, api_key: Optional[str], model_name: Optional[str]):
        ...
        # 创建LLM客户端
        if self.llm_provider in ["deepseek", "qwen", "openai"]:
            llm = ChatOpenAI(
                model=model,
                api_key=api_key,
                base_url=base_url if self.llm_provider != "openai" else None,
...
"""
        ...
        try:
            messages = [
                SystemMessage(content="你是一位资深的投资分析师，擅长综合不同投资理念。"),
